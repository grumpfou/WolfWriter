...
It has been written with Python 2.7, using the library PyQt4 and for a Windows platform (note that it should be easy to adapt it to another platform).
...
= Quick Start =
Open WolfWriterMainWindow.py. You have now a new book with only one chapter and only one scene. You can open the "QuatrevingtTreize.ww" archive file from the directtory Test. You have now the begining of the book Quatrevingt-Treize by VIctor Hugo (in French). 
	- on the left pannel you have the structure of the book
	- on the central pannel you have the current scene that you can modify 
	- on the right pannel you have the Encyclopedia with the names of the story.
Every name in color in the central pannel refers to an entry in the encyclopedia. Double click on one to acces the definition.
